...
final class DebounceManager: NSObject {
    ...
    ///简单的延时执行，只取最后一次的点击事件
    func debounceMethod(delay: TimeInterval = 1.0, _ block: @escaping () -> Void) {
        pendingBlock = block
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(handleCrop), object: nil)
        self.perform(#selector(handleCrop), with: nil, afterDelay: delay)
    }
