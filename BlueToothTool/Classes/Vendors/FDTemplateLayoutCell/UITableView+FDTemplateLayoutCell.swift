...
extension UITableView {
    ...
    func fd_systemFittingHeightForConfiguratedCell(_ cell: UITableViewCell) -> CGFloat {
        ...
        if fittingHeight == 0 {
                ...
                if cell.contentView.constraints.count > 0 {
                    let key = UnsafePointer(&Keys.fd_systemFittingHeightForConfiguratedCell)
                    if objc_getAssociatedObject(self, key) == nil && !cell.fd_usingFrameLayout {
    ...
    func fd_templateCell(for reuseIdentifier: String) -> UITableViewCell {
        ...
        let key = UnsafePointer(&Keys.fd_templateCell)
        ...
        if templateCellsByIdentifiers == nil {
            ...
            objc_setAssociatedObject(self, key, templateCellsByIdentifiers, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    ...
    public func fd_templateHeaderFooterView(for reuseIdentifier: String) -> UITableViewHeaderFooterView {
        ...
        let key = UnsafePointer(&Keys.fd_heightForHeaderFooterView)
        var templateHeaderFooterViews: [String: UITableViewHeaderFooterView]? = objc_getAssociatedObject(self, key) as? [String: UITableViewHeaderFooterView]
        if templateHeaderFooterViews == nil {
            templateHeaderFooterViews = [:]
            objc_setAssociatedObject(self, key, templateHeaderFooterViews, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
...
extension UITableViewCell {
    ...
    public var fd_isTemplateLayoutCell: Bool {
        set {
            let key = UnsafePointer(&Keys.isTemplateLayoutCell)
    ...
    public var fd_usingFrameLayout: Bool {
        set {
            let key = UnsafePointer(&Keys.enforceFrameLayout)
            objc_setAssociatedObject(self, key, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
