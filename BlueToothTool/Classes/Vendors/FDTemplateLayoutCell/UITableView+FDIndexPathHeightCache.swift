...
extension UITableView {
    ...
    var fd_indexPathHeightCache: FDIndexPathHeightCache {
        let key = UnsafePointer(&Keys.indexPathHeightCache)
        var cache = objc_getAssociatedObject(self, key) as? FDIndexPathHeightCache
        if cache == nil {
    ...
    var fd_keyedHeightCache: FDKeyedHeightCache {
        let key = UnsafePointer(&Keys.keyedHeightCache)
        var cache = objc_getAssociatedObject(self, key) as? FDKeyedHeightCache
        if cache == nil {
